...
class AccountFunctionalSpec extends GebSpec {
    ...
    def setupSpec() {
        ...
        remote {
            def account = new Account(username: 'account-test', email: 'test@me.com', password: 'password1')
    ...
    def "get account details (logged in as requested Account)" () {
        ...
        def resp = doGet('api/accounts/' + Account.findByUsername('account-test').id)
    ...
    def "cannot access account details for different user" () {
        ...
        def resp = doGet('api/accounts/' + Account.findByUsername('Joe Rockhead').id)
    ...
    def "update account email (logged in as requested Account)" () {
        ...
        def resp = doJsonPut('api/accounts/' + Account.findByUsername('account-test').id, [email: 'test@yahoo.com'])
    ...
    def "update account email denied for different user" () {
        ...
        def resp = doJsonPut('api/accounts/' + Account.findByUsername('Joe Rockhead').id, [email: 'test@yahoo.com'])
    ...
    def "deleting an account is not permitted (for any account)" () {
        ...
        def resp = doJsonDelete('api/accounts/' + Account.findByUsername('Joe Rockhead').id)
