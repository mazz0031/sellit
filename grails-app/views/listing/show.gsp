...
<div id="show-listing" class="content scaffold-show" role="main">
    ...
    <ol class="property-list listing">
        ...
        <g:if test="${listingInstance?.currentHighBid}">
            <li class="fieldcontain">
                <span id="bidAmount-label" class="property-label"><g:message code="bid.bidAmount.label" default="High Bid Amount"/></span>
                <span class="property-value" aria-labelledby="bidAmount-label"><g:fieldValue bean="${listingInstance}" field="currentHighBid"/></span>
            </li>
        </g:if>

        <g:if test="${listingInstance.endDate < new Date()}">
            <li class="fieldcontain">
                <span id="bidAccount-label" class="property-label"><g:message code="bid.bidAccount.label" default="High Bidder"/></span>
                <span class="property-value" aria-labelledby="bidAccount-label"><g:link controller="account" action="show" id="${listingInstance?.highBidAccount?.id}">${listingInstance?.highBidAccount?.name}</g:link></span>
            </li>
        </g:if>
